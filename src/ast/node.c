...
node_t *
node_make_await(position_t position, node_t *right){
	node_unary_t *node_unary;
	if(!(node_unary = (node_unary_t *)malloc(sizeof(node_unary_t)))){
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_unary_t));
		return NULL;
	...
	return node_make_node(position, NODE_KIND_AWAIT, node_unary);
}
...
node_make_cast(position_t position, node_t *cast, node_t *castable)
{
	...
	if(!(node_cast = (node_cast_t *)malloc(sizeof(node_cast_t))))
	{
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_cast_t));
		return NULL;
	}
	memset(node_cast, 0, sizeof(node_cast_t));
	node_cast->cast = cast;
	node_cast->castable = castable;
	
	return node_make_node(position, NODE_KIND_CAST, node_cast);
}
...
node_t *
node_make_iteration(position_t position, node_t *start_expression, node_t *step_expression, node_t *stop_expression)
{
	node_iteration_t *node_iteration;
	if(!(node_iteration = (node_iteration_t *)malloc(sizeof(node_iteration_t))))
	{
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_iteration_t));
		return NULL;
	}
	memset(node_iteration, 0, sizeof(node_iteration_t));
	node_iteration->start_expression= start_expression;
	node_iteration->step_expression = step_expression;
	node_iteration->stop_expression = stop_expression;
	
	return node_make_node(position, NODE_KIND_ITERATION, node_iteration);
}
...
node_t *
node_make_if(position_t position, node_t *condition, node_t *then_statement, node_t *else_statement){
	node_if_t *node_if;
	...
	node_if->then_statement = then_statement;
	node_if->else_statement = else_statement;
...
node_make_break(position_t position){
	return node_make_node(position, NODE_KIND_BREAK, NULL);
}

node_t *
node_make_continue(position_t position){
	return node_make_node(position, NODE_KIND_CONTINUE, NULL);
}
...
node_make_return(position_t position, node_t *expression)
{
	...
	if(!(node_return = (node_return_t *)malloc(sizeof(node_return_t))))
	{
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_return_t));
		return NULL;
	}
	memset(node_return, 0, sizeof(node_return_t));
	node_return->expression = expression;
	
	return node_make_node(position, NODE_KIND_RETURN, node_return);
}
...
node_make_throw(position_t position, node_t *expression)
{
	...
	node_throw->expression = expression;
...
node_make_type_parameter(position_t position, node_t *name, node_t *heritage, node_t *value){
	...
	node_type_parameter->heritage = heritage;
...
node_t *
node_make_func(position_t position, node_t *field, node_t *name, list_t *type_parameters, list_t *parameters, node_t *return_type, node_t *body)
{
...
node_t *
node_make_method(position_t position, uint64_t flag, list_t *fields, node_t *name, list_t *type_parameters, list_t *parameters, node_t *return_type, node_t *body)
{
...
node_make_import_paths(position_t position, list_t *list)
{
	...
	return node_make_node(position, NODE_KIND_IMPORT_PATHS, node_enumerable);
}

node_t *
node_make_import(position_t position, node_t *field, list_t *paths)
{
	...
	node_import->field = field;
	node_import->paths = paths;
	
	return node_make_node(position, NODE_KIND_IMPORT, node_import);
}

node_t *
node_make_async(position_t position, node_t *x)
{
	node_modifier_t *node_modifier;
	if(!(node_modifier = (node_modifier_t *)malloc(sizeof(node_modifier_t))))
	{
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_modifier_t));
		return NULL;
	}
	memset(node_modifier, 0, sizeof(node_modifier_t));
	node_modifier->x = x;
	
	return node_make_node(position, NODE_KIND_ASYNC, node_modifier);
}

node_t *
node_make_namespace(position_t position, node_t *name, list_t *body)
{
	node_namespace_t *node_namespace;
	if(!(node_namespace = (node_namespace_t *)malloc(sizeof(node_namespace_t)))){
		fprintf(stderr, "unable to allocted a block of %zu bytes\n", sizeof(node_namespace_t));
		return NULL;
	}
	memset(node_namespace, 0, sizeof(node_namespace_t));
	node_namespace->body = body;
	node_namespace->name = name;
	
	return node_make_node(position, NODE_KIND_NAMESPACE, node_namespace);
}
...
node_t *
node_make_export(position_t position, node_t *x)
{
	node_modifier_t *node_modifier;
	if(!(node_modifier = (node_modifier_t *)malloc(sizeof(node_modifier_t))))
	{
	...
	return node_make_node(position, NODE_KIND_EXPORT, node_modifier);
}
