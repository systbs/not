...
sy_execute_bitwise_or(sy_node_t *node, sy_strip_t *strip)
{
    if (node->kind == NODE_KIND_OR)
    {
        ...
        sy_record_t *left = sy_execute_bitwise_or(binary->left, strip);
        if (left == ERROR)
        {
            return ERROR;
        }

        sy_record_t *right = sy_execute_bitwise_xor(binary->right, strip);
        if (right == ERROR)
        {
            return ERROR;
        }
    ...
    {
        return sy_execute_bitwise_xor(node, strip);
    }
}
