...
sy_execute_bitwise_or(sy_node_t *node, sy_strip_t *strip, sy_node_t *applicant, sy_node_t *origin)
{
    if (node->kind == NODE_KIND_OR)
    {
        ...
        sy_record_t *left = sy_execute_bitwise_or(binary->left, strip, applicant, origin);
        if (left == ERROR)
        {
            return ERROR;
        }

        sy_record_t *right = sy_execute_bitwise_xor(binary->right, strip, applicant, origin);
        if (right == ERROR)
        {
            return ERROR;
        }

        sy_record_t *record = sy_execute_or(node, left, right, applicant);
    ...
    {
        return sy_execute_bitwise_xor(node, strip, applicant, origin);
    }
}
