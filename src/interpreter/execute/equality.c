...
sy_execute_eq(sy_node_t *node, sy_record_t *left, sy_record_t *right, sy_node_t *applicant)
{
    ...
    if (left->kind == RECORD_KIND_UINT8)
    {
        ...
        if (right->kind == RECORD_KIND_CHAR)
        {
            return sy_record_make_int8((*(uint8_t *)(left->value)) == (*(char *)(right->value)));
        }
        else
        if (right->kind == RECORD_KIND_STRING)
        {
    ...
    if (left->kind == RECORD_KIND_UINT16)
    {
        ...
        if (right->kind == RECORD_KIND_CHAR)
        {
            return sy_record_make_int8((*(uint16_t *)(left->value)) == (*(char *)(right->value)));
