...
void sy_scanner_set_token(sy_scanner_t *scanner, sy_token_t token)
{
...
void sy_scanner_destroy(sy_scanner_t *scanner)
{
	free(scanner);
}
...
sy_scanner_next(sy_scanner_t *scanner)
{
	if (scanner->reading_offset < strlen(scanner->source))
	{
		...
		if (r >= 0x80)
		{
			sy_utf8_decode(scanner->source + scanner->reading_offset, &r, &w);
			if (r == utf8_error && w == 1)
			{
				sy_error_lexer_by_position((sy_position_t){
											   .path = scanner->path,
