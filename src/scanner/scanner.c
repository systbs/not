...
scanner_next(scanner_t *scanner)
{
	if(scanner->reading_offset < strlen(scanner->source))
	{
		...
		if(r == 0)
		{
			scanner_error(scanner, (position_t){
...
scanner_advance(scanner_t *scanner)
{
    while (scanner->ch != eof) 
	{
			...
			if(scanner->ch == '\'')
			{
				...
				while(true) 
				{
					...
					if (ch == '\\') {
						...
						switch (scanner->ch) {
						case 'a': case 'b': case 'f': case 'n': case 'r': case 't': case 'v': case '\\': case '\'':
							scanner_next(scanner);
							break;
						...
						while (m > 0) {
							...
							if (d >= base) {
								...
								scanner_error(scanner, (position_t){
									...
									.column = scanner->column - (scanner->offset - start_offset),
									.line = scanner->line
								}, "illegal character %c in escape sequence", scanner->ch);
								return 0;
							}
							x = x*base + d;
							scanner_next(scanner);
							...
							else
							{
								if(isalpha(scanner->ch))
								{
									scanner_error(scanner, (position_t){
										...
										.column = scanner->column - (scanner->offset - start_offset),
										.line = scanner->line
									}, "wrong hexadecimal number format");
									return 0;
								}
								break;
							}
        ...
        else 
		{
			...
			else if(strncmp(scanner->source + start_offset, "public", max(length, 6)) == 0)
			{
            	scanner_set_token(scanner, (token_t)
				{
		        	.type = TOKEN_PUBLIC_KEYWORD, 
		        	.value = NULL, 
		        	.position = {
						.path = scanner->file_source->path,
						.offset = scanner->offset-length,
			...
			else if(strncmp(scanner->source + start_offset, "private", max(length, 7)) == 0)
			{
            	scanner_set_token(scanner, (token_t)
				{
		        	.type = TOKEN_PRIVATE_KEYWORD, 
		        	.value = NULL, 
		        	.position = {
						.path = scanner->file_source->path,
						.offset = scanner->offset-length,
			...
			else if(strncmp(scanner->source + start_offset, "namespace", max(length, 7)) == 0)
			{
            	scanner_set_token(scanner, (token_t)
				{
		        	.type = TOKEN_NAMESPACE_KEYWORD, 
		        	.value = NULL, 
		        	.position = {
						.path = scanner->file_source->path,
						.offset = scanner->offset-length,
