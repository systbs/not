...
parser_operator(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	...
	{
		if (parser_match(program, parser, TOKEN_TILDE) == -1)
		{
			return NULL;
		}
		operator = "~";
	...
	{
		if (parser_match(program, parser, TOKEN_NOT) == -1)
		{
			return NULL;
		}
		operator = "!";
	...
	{
		if (parser_match(program, parser, TOKEN_BACKSLASH) == -1)
		{
			return NULL;
		}
		operator = "\\";
	...
	{
		if (parser_match(program, parser, TOKEN_AND_AND) == -1)
		{
			return NULL;
		}
		operator = "&&";
	...
	{
		if (parser_match(program, parser, TOKEN_OR_OR) == -1)
		{
			return NULL;
		}
		operator = "||";
...
static node_t *
parser_lambda(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	node_t *node = node_create(scope, parent, parser->token->position);
	if (node == NULL)
	{
	...
	if (parser_match(program, parser, TOKEN_FUN_KEYWORD) == -1)
	{
		return NULL;
	}
	...
	if (parser->token->type == TOKEN_ID)
	{
		...
		{
			parser_error(program, parser->token->position, "wrong name");
			return NULL;
		}
	}
...
parser_primary(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	...
	if (parser->token->type == TOKEN_FUN_KEYWORD)
	{
		return parser_lambda(program, parser, scope, parent);
	}
	else 
	{
...
parser_postfix(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	...
	while (node2 != NULL)
	{
		if (parser->token->type == TOKEN_LT)
		{
			...
			node_t *concepts = parser_fields(program, parser, scope, parent);
			if (concepts == NULL)
			{
...
parser_expression(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	return parser_logical_or(program, parser, scope, parent);
}
...
parser_generic(program_t *program, parser_t *parser, node_t *scope, node_t *parent)
{
	...
	if (parser->token->type == TOKEN_EXTENDS_KEYWORD)
	{
		...
		type = parser_postfix(program, parser, scope, node);
		if (!type)
		{
			return NULL;
		}
...
parser_module(program_t *program, parser_t *parser)
{
	...
	while (parser->token->type != TOKEN_EOF)
	{
		...
		if (parser->token->type == TOKEN_EXPORT_KEYWORD)
		{
			item = parser_export(program, parser, node, node);
			if (item == NULL)
			{
				return NULL;
			}
		...
		{
			item = parser_import(program, parser, node, node);
			if (item == NULL)
			{
				return NULL;
			}
		...
		if (parser->token->type == TOKEN_ENUM_KEYWORD)
		{
			item = parser_enum(program, parser, node, node, PARSER_MODIFIER_NONE);
			if (item == NULL)
			{
				return NULL;
			}
		...
		{
			item = parser_class(program, parser, node, node, PARSER_MODIFIER_NONE);
			if (item == NULL)
			{
				return NULL;
			}
	...
	return node_make_module(node, parser->file_source->path, items);
}
