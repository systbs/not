...
static node_t *
graph_return_node_contain_object_by_name(symbol_t *current, char *name);

static node_t *
graph_return_node_contain_array_by_name(symbol_t *current, char *name);

static node_t *
graph_return_node_if_symbol_equal_by_name(symbol_t *current, char *name)
{
	node_t *node = current->declaration;
	node_basic_t *node_basic;
	node_basic = (node_basic_t *)node->value;
	if(strncmp(name, node_basic->value, max(strlen(node_basic->value), strlen(name))) == 0)
	{
		return node;
	}
	return NULL;
...
static node_t *
graph_return_node_separate_by_name(symbol_t *current, char *name)
{
	symbol_t *a;
...
static node_t *
graph_return_node_contain_name_by_name(symbol_t *current, char *name)
{
	symbol_t *a;
	for(a = current->begin; a != current->end; a = a->next)
	{
		...
		{
			node_t *node;
			node = graph_return_node_separate_by_name(a, name);
			if(node)
			{
				return node;
			}
...
static node_t *
graph_return_node_contain_object_property_by_name(symbol_t *current, char *name)
{
	symbol_t *a;
	for(a = current->begin; a != current->end; a = a->next)
	{
...
graph_return_node_contain_object_by_name(symbol_t *current, char *name)
{
	...
	for(a = current->begin; a != current->end; a = a->next)
	{
		...
		{
			node_t *node;
			node = graph_return_node_contain_object_property_by_name(a, name);
			if(node)
			{
				return node;
			}
