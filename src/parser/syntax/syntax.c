...
not_syntax_expected(not_syntax_t *syntax, int32_t type)
{
	...
	default:
		sprintf(
			...
			"expected '%s', got '%s'\n\tMajor:%s-%u",
			not_token_get_name(type),
			not_token_get_name(syntax->token->type), __FILE__, __LINE__);
		break;
	}
