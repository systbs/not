		...
		if (symbol_check_flag(a, SYMBOL_FLAG_TYPE_PARAMETER))
		{
			...
			for (b = target->begin;b != target->end;b = b->next)
			{
				if (symbol_check_flag(b, SYMBOL_FLAG_ARGUMENT))
				{
					...
					symbol_t *origin;
					origin = syntax_find_type_primary(graph, target, b, target, syntax_extract(b), NULL);
					if (!origin)
					{
...
{
	if (symbol_check_flag(target, SYMBOL_FLAG_NULL))
	{
		symbol_t *result;
	...
	if (symbol_check_flag(target, SYMBOL_FLAG_CHAR))
	{
		symbol_t *result;
	...
	if (symbol_check_flag(target, SYMBOL_FLAG_STRING))
	{
		symbol_t *result;
	...
	if (symbol_check_flag(target, SYMBOL_FLAG_NUMBER))
	{
		symbol_t *result;
	...
	if (symbol_check_flag(target, SYMBOL_FLAG_BOOLEAN))
	{
		symbol_t *result;
	...
	for (a = root->begin;(a != root->end);a = a->next)
	{
		...
		if (symbol_check_flag(a, SYMBOL_FLAG_CLASS))
		{
			symbol_t *result = syntax_subset_exist_in_flag(a, target, SYMBOL_FLAG_NAME);
			if (result && (result->id != target->id))
			{
...
syntax_find_type_primary(graph_t *graph, symbol_t *root, symbol_t *subroot, symbol_t *applicant, symbol_t *target, symbol_t *arguments)
{
	...
	if (
		symbol_check_flag(target, SYMBOL_FLAG_ID) || 
		symbol_check_flag(target, SYMBOL_FLAG_NULL) ||
		symbol_check_flag(target, SYMBOL_FLAG_CHAR) ||
		symbol_check_flag(target, SYMBOL_FLAG_STRING) ||
		symbol_check_flag(target, SYMBOL_FLAG_NUMBER) ||
		symbol_check_flag(target, SYMBOL_FLAG_BOOLEAN)
		)
	{
...
syntax_composite(graph_t *graph, symbol_t *root, symbol_t *subroot, symbol_t *current)
{
	...
	symbol_t *result;
	result = syntax_find_reference(graph, root, subroot, root, current);
	if (!result)
	{
		syntax_error(graph, current, "reference of composite not found");
		return NULL;
	}
	return result;
}
...
syntax_call(graph_t *graph, symbol_t *root, symbol_t *subroot, symbol_t *current)
{
	...
	if (!result)
	{
		syntax_error(graph, current, "reference of call not found");
		return NULL;
	}
	return result;
}
			...
			if (symbol_check_flag(a, SYMBOL_FLAG_LEFT))
			{
				...
				for (b = a->begin;(b != a->end); b = b->next)
				{
						...
						symbol_check_flag(b, SYMBOL_FLAG_ELLIPSIS))
					{
						...
						if (symbol_check_flag(b, SYMBOL_FLAG_TYPEOF))
						{
							return result;
						}
					}
...
syntax_method(graph_t *graph, symbol_t *root, symbol_t *subroot, symbol_t *current)
{
	...
	for (a = current->begin; a != current->end; a = a->next)
	{
		...
		{
			symbol_t *result;
			result = syntax_find_reference_primary(graph, root, subroot, root, syntax_extract(a), NULL, NULL);
			if (result)
			{
